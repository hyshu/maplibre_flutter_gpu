...
for scene in "${scenes[@]}"; do
  ...
  if [[ "$seen_scenes" == *"|$scene|"* ]]; then
    echo "Duplicate macOS visual scene: $scene" >&2
    exit 2
  fi
...
if [[ "${#scenes[@]}" -eq 0 ]]; then
  echo "At least one macOS visual scene is required." >&2
  exit 2
fi
...
capture_dir="$(mktemp -d "${TMPDIR:-/tmp}/maplibre-macos-e2e.XXXXXX")"
cleanup() {
  rm -rf "$capture_dir"
}
trap cleanup EXIT
...
for scene in "${scenes[@]}"; do
  ...
  scene_baseline="$repository_root/e2e/visual/baseline/macos-$scene.png"
  if [[ -n "$baseline" ]]; then
  ...
  if [[ "$update_baseline" -eq 1 ]]; then
    ...
    if [[ "$allow_missing_baseline" -eq 1 && ! -f "$scene_baseline" ]]; then
      echo "WARNING: no macOS baseline at $scene_baseline." >&2
    else
      set +e
      (
        cd "$repository_root/e2e/visual/runner"
